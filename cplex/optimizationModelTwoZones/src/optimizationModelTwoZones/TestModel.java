...
public class TestModel {
	public static void main(String[] args) {
		...
		try {
			...
			IloLinearNumExpr[][] StorageContentInflow = new IloLinearNumExpr[ZONES][HOUR];
			for (int i = 0; i < ZONES; i++) {
				for (int j = 1 ; j < HOUR; j++){
					StorageContentInflow[i][j] = cplex.linearNumExpr();
					StorageContentInflow[i][j].addTerm(1.0, E[i][j - 1]);
					StorageContentInflow[i][j].addTerm(n, sIn[i][j - 1]);
				}
			}
			
			IloLinearNumExpr[][] StorageContentOutflow = new IloLinearNumExpr[ZONES][HOUR];
			for (int i = 0; i < ZONES; i++) {
				for (int j = 1 ; j < HOUR; j++){
					StorageContentOutflow[i][j] = cplex.linearNumExpr();
					StorageContentOutflow[i][j].addTerm(1.0, E[i][j - 1]);
					StorageContentOutflow[i][j].addTerm(-nInv, sOut[i][j - 1]);
				}
			}
			...
			for(int j = 0; j < ZONES; j++){ 
				for (int i = 1; i < HOUR; i++){
					cplex.addEq(StorageContentOutflow[i][j], E[i][j]);
				}
			}
